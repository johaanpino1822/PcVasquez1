...
// Utilidad para obtener URLs de imágenes
...
const ProductCard = ({ 
  ...
  const handleUpdateQuantity = useCallback((e, newQuantity) => {
    ...
    if (isAdded) onUpdateQuantity(product._id, newQuantity);
  }, [product.stock, product._id, isAdded, onUpdateQuantity]);
  ...
  return (
    ...
    >
      <div className="absolute inset-0 mt-16 bg-gradient-to-br from-[#0C4B45]/10 to-[#83F4E9]/10 rounded-3xl shadow-lg transform group-hover:-translate-y-2 transition-transform duration-300"></div>
...
const Pagination = ({ currentPage, totalPages, totalProducts, productsPerPage, onPageChange }) => {
...
const FiltersSidebar = ({ categories, categoryFilter, setCategoryFilter, sortOption, setSortOption, mobileFiltersOpen, setMobileFiltersOpen }) => (
  <>
    <button
      type="button"
...
const QuickViewModal = ({ product, onClose, isFavorite, onToggleFavorite, cart, onAddToCart, onUpdateQuantity }) => {
          ...
          <div className="grid md:grid-cols-2 gap-10">
            ...
            <div>
              ...
              <div className="mb-8">
                ...
                {discount > 0 && (
                  <span className="ml-3 bg-[#F2A9FD]/20 text-[#662D8F] text-sm font-bold px-3 py-1 rounded-full">
                    {discount}% OFF
                  </span>
                )}
              </div>
      ...
      <AnimatePresence>
        {quickViewProduct && (
          <QuickViewModal
            ...
            isFavorite={favorites.includes(quickViewProduct._id)}
            onToggleFavorite={toggleFavorite}
            cart={cart}
            onAddToCart={handleAddToCart}
            onUpdateQuantity={handleUpdateQuantity}
